...
/* ======================================================
   ...
   ====================================================== */
function goTo(page) {
  // hentikan audio percakapan kalau user pindah dari halaman Percakapan
  ...
  if (page === 'irodori') (initIrodorigoi(), endQuiz()); //Tambahan initIrodorigoi()
  if (page === 'percakapan') (initPercakapan(), endQuiz());
  if (page === 'choukai') { initChoukai(); endQuiz(); }
  if (page === 'choukai-bab') { renderChoukaiBab(); endQuiz(); }
  if (page === 'choukai-audio') { renderChoukaiAudio(); endQuiz(); }
...
function renderChoukaiBab() {
  const grid = document.getElementById('choukaiBabGrid');
  const title = document.getElementById('choukaiBabTitle');
  const data = irodoriAudioData[choukaiLevel];
  if (!data) return;

  title.textContent = data.name + ' — Pilih Bab';
  const chapters = Object.keys(data.chapters).sort((a, b) => Number(a) - Number(b));

  grid.innerHTML = chapters.map((bab, i) => {
    const count = data.chapters[bab].length;
    return `
      <button class="choukai-bab-btn" onclick="selectChoukaiBab(${bab})" role="button" tabindex="0" aria-label="Bab ${bab}">
        <span class="choukai-bab-num">${bab}</span>
        <span class="choukai-bab-label">${count} audio</span>
      </button>
    `;
  }).join('');
}

function selectChoukaiBab(bab) {
  choukaiBab = bab;
  goTo('choukai-audio');
}

function renderChoukaiAudio() {
  const list = document.getElementById('choukaiAudioList');
  const title = document.getElementById('choukaiAudioTitle');
  const data = irodoriAudioData[choukaiLevel];
  if (!data || choukaiBab == null) return;

  const audios = data.chapters[choukaiBab] || [];
  title.textContent = data.name + ' — Bab ' + choukaiBab;

  if (audios.length === 0) {
    list.innerHTML = `
      <div class="choukai-audio-empty">
        <i class="fas fa-music"></i>
        <p>Tidak ada audio untuk Bab ini.</p>
      </div>
    `;
    return;
  }

  list.innerHTML = audios.map((item, i) => `
    <div class="choukai-audio-item" style="animation-delay:${i * 0.04}s">
      <div class="choukai-audio-header">
        <span class="choukai-audio-label">${item.label}</span>
        <a href="${item.url}" download class="choukai-audio-download" title="Download" aria-label="Download ${item.label}">
          <i class="fas fa-download"></i>
        </a>
      </div>
      <div class="choukai-audio-player-wrap">
